...
import com.example.chatapplication.presentation.channel.components.CustomChannelList
    ...
    ChatTheme(
        ...
        Box(modifier = Modifier.fillMaxSize()) {
            Column(modifier = modifier.fillMaxSize()) {
                ...
                //TODO :Filter Channel
                val currentUser = client.getCurrentUser()!!.id
                val filters = Filters.and(
                    Filters.eq("type", "messaging"),
                    Filters.`in`("members", listOf(currentUser)),
                    Filters.eq("starAligned", true)
                )
                CustomChannelList(
                    viewModel = viewModel(
